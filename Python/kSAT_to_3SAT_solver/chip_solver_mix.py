...
def chip_solver_mix(cnffile,cnffile_new, scheme):
    
    
    Clauses, var, nc, k_sat, varo = k_sat_read_cnf.k_sat_read_cnf(cnffile, cnffile_new, scheme)
    ...
    node_set = spectral_analysis.spectral_analysis(A, N)
    ...
    VC = 1
    small_eps = 0.00
    X_final = np.random.binomial(1, 0.5, S)  # Generate initial state
    vtmpb = 100
    c = 1
    time_arr = [0]*parts
    total_time = []
    ...
    while vtmpb != 0:
        ...
        if c<=np.ceil(10000/parts):
        
            start_flip_time = time.time()
            flip_nodes = []
        
            cl_matrix = np.zeros((S, S))
            cl_unsatis = np.zeros(S)
            ...
            flip_nodes = [i for i in range(S) if cl_unsatis[i] < vtmpt]
            ...
            if all(np.array_equal(x, flip_node_arr[0]) for x in flip_node_arr) or all(unsatis_arr[-5:] == unsatis_arr[-1]):
                X_final = np.random.binomial(1, 0.5, S)
                continue
            
            if not flip_nodes:
                equal = [i for i in range(S) if cl_unsatis[i] == vtmpt]
                if equal:
            ...
            K1 = 10**8
            Clauses_k, var, C = read_cnf.read_cnf(cnffile, k_sat)
            CMat = np.zeros((C, var), dtype=int)
            
            for j in range(var):
                for k in range(C):
                    if j+1 in Clauses_k[k]:
                        CMat[k, j] = 1
                    if -(j+1) in Clauses_k[k]:
                        CMat[k, j] = -1
            
            flip_time.append(time.time() - start_flip_time)
            X, time_to_sat = MCMC_SAT.MCMC_SAT(CMat, var, C, K1)
            cv_time = time.time()
            v = compute_vars.compute_vars(X, S, varo, Clauses_k, C, k_sat, Clauses_org)
            v[:len(X)] = X
            X_final = v
            flip_time.append(time.time() - cv_time + time_to_sat)
    ...
    return X_final,sat_time,flip_time,total_time,MCMC_time
