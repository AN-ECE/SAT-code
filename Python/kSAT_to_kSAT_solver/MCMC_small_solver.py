...
def MCMC_small_solver(N, K, VC, small_eps, X_final, node_set, Clause_set, p):
    N = len(node_set[p])
    X = X_final[[x - 1 for x in node_set[p]]]
    nc = len(Clause_set[p])
    ...
    for i in range(1, K+1):
        ...
        vtmpj, vtmp = rnode_chip.rnode_chip(N, X, small_eps, Clause_set[p], nc)
        ...
        for j in r:
            if j == r[Ival]:  # Node that switches
                L[j] = 0
                X[j] = 1 - X[j]  # Toggle between 0 and 1
            else:
