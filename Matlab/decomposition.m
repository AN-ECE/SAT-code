...
function [node_set,Clause_set,Clause_inter,clause_comm_nodes,s_node]=decomposition(start_comb,chip_size,S,A,Clauses)
...
if (size(G,2)<=50)
    ...
    set1 = [set1, inter(1:(228 - size(set1,2)))];
    set2=[];
...
if r==1
    s1 = cellfun(@length, allnode11);
    s2 = cellfun(@length, allnode21);

    p=find((s1 <=50) & (s2<=50)& max(min(s1,s2)),1);
    if isempty(p)
...
elseif r==2
    s1 = cellfun(@length, allnode12);
    s2 = cellfun(@length, allnode22);
    p=find((s1 <=50) & (s2<=50)& max(min(s1,s2)),1);
    if isempty(p)
...
elseif r==3
    s1 = cellfun(@length, allnode13);
    s2 = cellfun(@length, allnode23);
    p=find((s1 <=50) & (s2<=50)& max(min(s1,s2)),1);
    if isempty(p)
...
elseif r==4
    ...
    s1 = cellfun(@length, allnode14);
    s2 = cellfun(@length, allnode24);
    p=find((s1 <=50) & (s2<=50)& max(min(s1,s2)),1);
    if isempty(p)
...
Clause_set={Clause_set1;Clause_set2};
node_set={G_chip1,G_chip2};
